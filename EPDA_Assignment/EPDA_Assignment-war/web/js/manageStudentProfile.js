// Form validation functions for Register Student form
...
function validateForm() {
    ...
    isValid = validateIntakeYear() && isValid;
    isValid = validateCurrentLevel() && isValid;
...
function validateIntakeYear() {
    const intakeYear = document.getElementById('intakeYear');
    const errorRow = document.getElementById('intakeYear-error');
    const errorMessage = document.getElementById('intakeYear-error-message');
    
    if (!intakeYear.value.trim()) {
        showError(errorRow, errorMessage, 'Intake year is required', intakeYear);
        return false;
    }
    
    const year = parseInt(intakeYear.value.trim());
    const currentYear = new Date().getFullYear();
    
    if (isNaN(year)) {
        showError(errorRow, errorMessage, 'Please enter a valid year', intakeYear);
        return false;
    }
    
    if (!/^\d{4}$/.test(intakeYear.value.trim())) {
        showError(errorRow, errorMessage, 'Year must be 4 digits', intakeYear);
        return false;
    }
    
    if (year < 2000 || year > currentYear + 1) {
        showError(errorRow, errorMessage, `Intake year must be between 2000 and ${currentYear + 1}`, intakeYear);
        return false;
    }
    
    hideError(errorRow, intakeYear);
    return true;
}
...
function validateCurrentLevel() {
    ...
    const errorRow = document.getElementById('currentLevel-error');
    const errorMessage = document.getElementById('currentLevel-error-message');
    
    if (!currentLevel.value) {
        showError(errorRow, errorMessage, 'Please select a current level', currentLevel);
        return false;
    }
    
    hideError(errorRow, currentLevel);
    return true;
}
