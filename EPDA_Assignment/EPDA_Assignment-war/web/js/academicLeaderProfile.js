// Form validation functions for Add Academic Leader form
...
function validateForm() {
    ...
    isValid = validateLeaderRole() && isValid;
...
function validateLeaderRole() {
    const leaderRole = document.getElementById('leaderRole');
    const errorRow = document.getElementById('leaderRole-error');
    const errorMessage = document.getElementById('leaderRole-error-message');
    
    if (!leaderRole.value) {
        showError(errorRow, errorMessage, 'Please select a leader role', leaderRole);
        return false;
    }
    
    hideError(errorRow, leaderRole);
    return true;
}
...
function validateEndDate() {
    ...
    // End date is optional, so only validate if it's filled
    if (endDate.value) {
        if (!startDate.value) {
            showError(errorRow, errorMessage, 'Please select a start date first', endDate);
            return false;
        }
        ...
        if (end <= start) {
            showError(errorRow, errorMessage, 'End date must be after start date', endDate);
            return false;
        }
    }
